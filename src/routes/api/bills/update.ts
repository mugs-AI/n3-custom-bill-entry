...
// Server-side Purchase Invoice updater. Mirrors bills/create.ts but calls the
// N3 Update endpoint and preserves existing detail line ids so N3 treats the
// operation as an in-place edit rather than a delete-and-recreate.
//
// Duplicate check excludes the invoice being updated so re-saving with the
// same Supplier INV# does not self-conflict.
...
type UpdateResult =
  | { ok: true; docCode: string; id: string | null; message: string }
  | {
      ok: false;
...
function jsonRes(status: number, body: UpdateResult): Response {
...
function coercePayload(raw: unknown): UpdatePayload | { error: string } {
  if (!raw || typeof raw !== "object") return { error: "Missing body" };
  const r = raw as Record<string, unknown>;
  const invoiceId = r.invoiceId;
  if (typeof invoiceId !== "string" || !invoiceId) return { error: "invoiceId is required" };
  ...
  const outLines: UpdatePayload["lines"] = [];
  for (let i = 0; i < lines.length; i++) {
    ...
    const taxRateFactor =
      isFiniteNumber(rawRateFactor) && rawRateFactor >= 0 && rawRateFactor <= 1
        ? rawRateFactor
        : 0;
    const n3Id = typeof l.n3Id === "string" && l.n3Id ? l.n3Id : null;
...
async function checkDuplicate(
  ...
  for (let p = 0; p < MAX_PAGES; p++) {
    ...
    for (const row of rows) {
      ...
      if (row.id === excludeInvoiceId) continue;
...
async function handle(request: Request): Promise<Response> {
  ...
  // Fetch the current PI so we send N3 back a full document with any fields
  // we don't expose in the UI (currencyId, stockLocationId, etc.) preserved.
  const existing = await n3Get<Record<string, unknown>>(
    base,
    token,
    `api/PurchaseInvoices/${encodeURIComponent(parsed.invoiceId)}`,
  );
  if (!existing.ok) {
    if (existing.status === 401)
      return jsonRes(401, { ok: false, kind: "auth", error: existing.message });
    return jsonRes(502, {
      ok: false,
      kind: "n3",
      error: `Load existing PI failed: ${existing.message}`,
    });
  }
  const base0 = existing.data ?? {};

  const round2 = (n: number) =>
    Number.isFinite(n) ? (Math.sign(n) * Math.round(Math.abs(n) * 100)) / 100 : 0;

  const itemDetails = parsed.lines.map((l, i) => {
  ...
  else delete payload.purchaserId;

  const updated = await n3Post<Record<string, unknown>>(
    base,
    token,
    "api/PurchaseInvoices/Update",
    payload,
  );
  if (!updated.ok) {
    if (updated.status === 401)
      return jsonRes(401, { ok: false, kind: "auth", error: updated.message });
    return jsonRes(502, { ok: false, kind: "n3", error: updated.message });
  }
  const data = updated.data;
  const docCode =
    typeof data.docCode === "string" && data.docCode
      ? data.docCode
      : (typeof base0.docCode === "string" ? base0.docCode : "");
  const id = typeof data.id === "string" ? data.id : parsed.invoiceId;
  if (!docCode) {
    return jsonRes(502, {
      ok: false,
      kind: "n3",
  ...
  return jsonRes(200, { ok: true, docCode, id, message: updated.message });
}

export const Route = createFileRoute("/api/bills/update")({
  server: {
    handlers: {
